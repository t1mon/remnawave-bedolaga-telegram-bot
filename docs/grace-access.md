...
Миграция имеет собственный revision ID `grace_access_0097`, чтобы не столкнуться по имени с будущей upstream-миграцией `0097`. Если upstream также добавит ветку после `0096`, у Alembic появятся две головы. Тогда перед обновлением нужна маленькая merge migration либо перенос `down_revision` grace-миграции на новую актуальную голову. Сам модуль grace при этом переносится отдельно, а точки интеграции остаются небольшими.
